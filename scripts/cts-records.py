...
if __name__ == '__main__':
    ...
    if config.dedup:
        eds = raw.filter(f.rlike('book', lit(r'-grc')) | f.rlike('book', lit(r'-lat'))
                ).groupBy('book'
                ).count(
    ...
    raw.filter(col('text').isNotNull()
      ).filter(f.rlike('book', lit(r'-grc')) | f.rlike('book', lit(r'-lat'))
      ).withColumn('text', nfd_norm('text')
      ).withColumn('parts', f.split(f.split('loc', ':')[4], r'\.')
      ).withColumn('top', f.when((f.size('parts') >= 2) | ((f.size('parts') >= 1) & (col('book') == 'urn:cts:latinLit:stoa0119.stoa003.opp-lat1')),
