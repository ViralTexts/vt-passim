...
if __name__ == "__main__":
    ...
    gap = 730
    odir = sys.argv[2]
    vocabFile = odir + "/vocab.gz"
    featFile = odir + "/feats.parquet"

    try:
        full = sqlContext.read.load(featFile)
        vocab = np.loadtxt(vocabFile, 'string')
    except:
        featurizeData(sqlContext.read.load(sys.argv[1]), gap, vocabFile, featFile)
        full = sqlContext.read.load(featFile)
        vocab = np.loadtxt(vocabFile, 'string')
    ...
    for i in range(iter):
        ...
        w -= rate * update
        print(update)
        np.savetxt("%s/iter%03d.gz" % (odir, i), w)
