...
if __name__ == '__main__':
    ...
    patch_alto = udf(lambda fin, fout, lines: patchAlto(fin, config.outputPath+'/'+fout, lines),
    ...
    sample = raw.withColumn('book', f.regexp_replace('id', '/[^/]+$', '')
                ...
                ).agg(slice(sort_array(collect_list(struct('matchRate', 'id', 'lineID')), False),
                            1, config.lines).alias('lines')
                ).select(explode('lines').alias('line')
                ).select(col('line.id'), col('line.lineID'))

    lines = raw.join(sample, ['id', 'lineID'], 'left_semi'
                ).groupBy('id'
